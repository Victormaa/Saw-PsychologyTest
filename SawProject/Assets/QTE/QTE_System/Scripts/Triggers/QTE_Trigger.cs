...
//function that returns the string name of the input to use, and loads it's sprite.
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (IsCollider)
        {
            ...
            collision.GetComponent<PlayerController>().Mashring.gameObject.SetActive(true);
            collision.GetComponent<PlayerController>().Mashcircle.gameObject.SetActive(true);
            if (collision.CompareTag("Player1"))
            {
                TriggerEnabled = true;
                QTE_main.Singleton.ButtonPositions = ButtonPositions;
                var a = collision.GetComponent<PlayerController>().Adefaultpos.position;
                //set meshring and circle's position to the position
                collision.GetComponent<PlayerController>().Mashring.position = a;
                collision.GetComponent<PlayerController>().Mashcircle.position = a;

                collision.GetComponent<PlayerController>().Amovable = false;
                //make player could not move
            ...
            if (collision.CompareTag("Player2"))
            {
                TriggerEnabled = true;
                QTE_main.Singleton.ButtonPositions = ButtonPositions;
                var b = collision.GetComponent<PlayerController>().Bdefaultpos.position;
                //set meshring and circle's position to the position
                collision.GetComponent<PlayerController>().Mashring.position = b;
                collision.GetComponent<PlayerController>().Mashcircle.position = b;

                collision.GetComponent<PlayerController>().Bmovable = false;
                //make player could not move
                QTE();
            }
        }
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Player2") )
        {
            if (QTE_main.Singleton.MashValue >= 1.0f)
            {
                collision.GetComponent<PlayerController>().Bmovable = true;
                Destroy(this.GetComponent<QTE_Trigger>());
                collision.GetComponent<PlayerController>().PlayerUnlocked();
                collision.GetComponent<PlayerController>().isfootlock = false;
            }
        }
        ...
        if (collision.CompareTag("Player1"))
        {
            if (QTE_main.Singleton.MashValue >= 1.0f)
            {
                collision.GetComponent<PlayerController>().Amovable = true;
                Destroy(this.GetComponent<QTE_Trigger>());
                collision.GetComponent<PlayerController>().PlayerUnlocked();
                collision.GetComponent<PlayerController>().isfootlock = false;
            }
    ...
    private void OnTriggerExit2D(Collider2D collision)
    {
        ...
        if (collision.CompareTag("Player1"))
        {
            collision.GetComponent<PlayerController>().Mashring.gameObject.SetActive(false);
            collision.GetComponent<PlayerController>().Mashcircle.gameObject.SetActive(false);

...
public class QTE_Response_MashUI : MonoBehaviour
{
	...
	void Update ()
	{
		...
		if (QTE_main.Singleton.TriggeringObject == this.gameObject) {
			...
			QTE_main.Singleton.MashCircle.StartAtZeroScale = StartAtZeroScale;
			QTE_main.Singleton.MashCircle.MaxSize = MaxSize;
			QTE_main.Singleton.MashCircle.Mashing = true;
